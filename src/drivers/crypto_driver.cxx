 ...
 */
std::vector<unsigned char> CryptoDriver::encrypt_and_tag(SecByteBlock AES_key,
    ...
    std::string to_tag =
        std::string((const char *)encrypted.second.data(), encrypted.second.size()) +
        encrypted.first;
 ...
 */
std::pair<std::vector<unsigned char>, bool> CryptoDriver::decrypt_and_verify(
    SecByteBlock AES_key, SecByteBlock HMAC_key, std::vector<unsigned char> ciphertext_data) {
    // Deserialize
    HMACTagged_Wrapper ciphertext;
    ciphertext.deserialize(ciphertext_data);

    // Verify HMAC
    std::string to_verify = std::string((const char *)ciphertext.iv.data(), ciphertext.iv.size()) +
 ...
 */
SecByteBlock CryptoDriver::DH_generate_shared_key(const DH &DH_obj,
                                                  const SecByteBlock &DH_private_value,
                                                  const SecByteBlock &DH_other_public_value) {
    SecByteBlock DH_shared_key(DH_obj.AgreedValueLength());
    SecByteBlock private_key_real =
        SecByteBlock(NULL, DH_obj.PrivateKeyLength() - DH_private_value.size()) + DH_private_value;
    SecByteBlock other_key_real =
        SecByteBlock(NULL, DH_obj.PublicKeyLength() - DH_other_public_value.size()) +
...
SecByteBlock CryptoDriver::AES_generate_key(const SecByteBlock &DH_shared_key) {
    ...
    SecByteBlock aes_salt((const unsigned char *)(aes_salt_str.data()), aes_salt_str.size());
    // Derive AES key using HKDF
    SecByteBlock AES_shared_key(AES::DEFAULT_KEYLENGTH);
    HKDF<SHA256> hkdf;
    hkdf.DeriveKey(AES_shared_key, AES_shared_key.size(), DH_shared_key, DH_shared_key.size(),
                   aes_salt, aes_salt.size(), NULL, 0);
 ...
 */
std::pair<std::string, SecByteBlock> CryptoDriver::AES_encrypt(SecByteBlock key,
                                                               std::string plaintext) {
    try {
        ...
        StringSource ss1(plaintext, true,
                         new StreamTransformationFilter(AES_encryptor, new StringSink(ciphertext)));

        return std::make_pair(ciphertext, iv);
    } catch (CryptoPP::Exception &e) {
        std::cerr << e.what() << std::endl;
        std::cerr << "This function was likely called with an incorrect shared key." << std::endl;
 ...
 */
std::string CryptoDriver::AES_decrypt(SecByteBlock key, SecByteBlock iv, std::string ciphertext) {
    try {
        ...
        StringSource ss1(ciphertext, true,
                         new StreamTransformationFilter(AES_decryptor, new StringSink(recovered)));
        return recovered;
    } catch (CryptoPP::Exception &e) {
        std::cerr << e.what() << std::endl;
        std::cerr << "This function was likely called with an incorrect shared key." << std::endl;
 ...
 */
SecByteBlock CryptoDriver::HMAC_generate_key(const SecByteBlock &DH_shared_key) {
    std::string hmac_salt_str("salt0001");
    SecByteBlock hmac_salt((const unsigned char *)(hmac_salt_str.data()), hmac_salt_str.size());
    // Derive 256 byte ECB key from DH shared key using KDF
    HKDF<SHA256> hkdf;
    SecByteBlock HMAC_shared_key(SHA256::BLOCKSIZE);
    hkdf.DeriveKey(HMAC_shared_key, HMAC_shared_key.size(), DH_shared_key, DH_shared_key.size(),
                   hmac_salt, hmac_salt.size(), NULL, 0);
 ...
 */
std::string CryptoDriver::HMAC_generate(SecByteBlock key, std::string ciphertext) {
    try {
        std::string mac;
 ...
 */
bool CryptoDriver::HMAC_verify(SecByteBlock key, std::string ciphertext, std::string mac) {
...
/**
 * Hash inputs. SHA256(input)
 */
CryptoPP::SecByteBlock CryptoDriver::hash(CryptoPP::SecByteBlock &input) {
    CryptoPP::SHA256 hash;
    CryptoPP::SecByteBlock digest(hash.DigestSize());
    hash.Update(input.BytePtr(), input.size());
