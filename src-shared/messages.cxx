...
// ================================================
// PSI Messages between sender and receiver
// ================================================

void SenderToReceiver_PSI_Message::serialize(std::vector<unsigned char> &data) {
    // Add message type.
    data.push_back((char)MessageType::SenderToReceiver_PSI_Message);
    ...
    size_t num_inputs = this->hashed_sender_inputs.size();
    std::memcpy(&data[idx], &num_inputs, sizeof(size_t));

    for (int i = 0; i < num_inputs; i++) {
        auto entry = byteblock_to_string(this->hashed_sender_inputs[i]);
        put_string(entry, data);
    }
}

int SenderToReceiver_PSI_Message::deserialize(std::vector<unsigned char> &data) {
    // Check correct message type.
    assert(data[0] == MessageType::SenderToReceiver_PSI_Message);

    // Get fields.
    this->type = (PSIType)data[1];

    size_t num_inputs;
    std::memcpy(&num_inputs, &data[2], sizeof(size_t));
    int n = 2 + sizeof(size_t);

    for (int i = 0; i < num_inputs; i++) {
        std::string entry;
        n += get_string(&entry, data, n);
        this->hashed_sender_inputs.push_back(string_to_byteblock(entry));
    }

    return n;
}

void ReceiverToSender_PSI_Message::serialize(std::vector<unsigned char> &data) {
    // Add message type.
    data.push_back((char)MessageType::ReceiverToSender_PSI_Message);
    ...
    num_inputs = this->hashed_receiver_inputs.size();
    std::memcpy(&data[idx], &num_inputs, sizeof(size_t));

    for (int i = 0; i < num_inputs; i++) {
        auto entry = byteblock_to_string(this->hashed_receiver_inputs[i]);
        put_string(entry, data);
    }
}

int ReceiverToSender_PSI_Message::deserialize(std::vector<unsigned char> &data) {
    // Check correct message type.
    assert(data[0] == MessageType::ReceiverToSender_PSI_Message);
    ...
    std::memcpy(&num_inputs, &data[n], sizeof(size_t));
    n += sizeof(size_t);
    for (int i = 0; i < num_inputs; i++) {
        std::string entry;
        n += get_string(&entry, data, n);
        this->hashed_receiver_inputs.push_back(string_to_byteblock(entry));
    }
